...
export default async function handler(
  ...
  const { content, systemPrompt, aiName, userCallName, providerId, model, apiKey } =
  ...
  const userText = `以下は今日の日記です。要約・気づき・ひとことフィードバックをお願いします。\n\n---\n${content}\n---`
  ...
  try {
    ...
    if (normalizedProviderId === 'anthropic') {
      ...
      return res.status(400).json({
        error: `未対応のAIプロバイダです: ${normalizedProviderId}`,
      })
    }
    ...
    if (!response.ok) {
      ...
      const msg = isAuthError
        ? `${providerName} API キーが無効です（${rawMsg}）。設定画面でキーを貼り直し、先頭の Bearer や余分な空白がないことを確認してください。`
        : rawMsg
