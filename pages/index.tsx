...
import { isFeatureEnabled, createStorageAdapter, getSettings, mergeVaultPersona } from '@/profiles'
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // vault からクイックリンクと ai_persona を取得
    if (p.gh_pat && p.github_repo) {
      ...
      adapter
        ...
        .then((config) => {
          ...
          // vault の ai_persona でプロファイルを上書き（空値は無視）
          if (config.ai_persona) {
            setProfile((prev) => prev ? mergeVaultPersona(prev, config.ai_persona!) : prev)
          }
        })
        .catch(() => {})
    }
